 ...
 * @returns {Promise<string>} Public URL of uploaded banner image
 */
 ...
 * Extracts base64 images from HTML and uploads them to Supabase Storage
 * @param {string} htmlContent - HTML content with potential base64 images
 * @param {string} blogId - Blog ID for organizing images
 * @returns {Promise<string>} Updated HTML content with Supabase Storage URLs
 */
export const uploadContentImages = async (htmlContent, blogId = null) => {
	try {
		...
		for (const img of images) {
			...
			// Skip if already a URL (not base64) - check for Supabase Storage URLs or external URLs
			...
			if (src.startsWith('data:')) {
				try {
					...
					const supabaseUrl = await uploadImageFromDataUrl(src, path);
					
					// Replace base64 URL with Supabase Storage URL in content
					updatedContent = updatedContent.replace(src, supabaseUrl);
				} catch (error) {
